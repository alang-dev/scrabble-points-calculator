...
export class ViewTopScoresPage extends BasePage {
  ...
  async verifyTableHeaders(headers: string[]) {
    for (const header of headers) {
      await expect(this.modal.locator('th').filter({ hasText: new RegExp(header, 'i') })).toBeVisible();
    }
  }

  async verifyTableData(data: Array<Record<string, string | number>>) {
    for (const row of data) {
      for (const [key, value] of Object.entries(row)) {
        await expect(this.modal.locator('td').filter({ hasText: String(value) })).toBeVisible();
      }
    }
  }
