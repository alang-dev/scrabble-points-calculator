...
export class ScoringRulesPage extends BasePage {
  ...
  async verifyTableData(data: Record<string, string[]>) {
    ...
    for (const [points, letters] of Object.entries(data)) {
      const expectedText = letters.join(', ');
      const scoreRow = scoringRulesTable.locator('tr').filter({ hasText: points }).filter({ hasText: letters[0] });
      await expect(scoreRow).toContainText(expectedText);
    }
  }
