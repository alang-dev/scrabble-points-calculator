...
import { ScoringRule } from '../test-data/scoring-rules'
...
export class ScoringRulesPage extends BasePage {
  ...
  async verifyTableHeaders(headers: string[]) {
    for (const header of headers) {
      await expect(
        this.page.locator('th').filter({ hasText: new RegExp(header, 'i') })
      ).toBeVisible()
    }
  }

  async verifyTableData(data: ScoringRule[]) {
    ...
    for (const rule of data) {
      const pointsCell = scoringRulesTable
        .locator('td')
        .filter({ hasText: new RegExp(`^${rule.points}$`) })
