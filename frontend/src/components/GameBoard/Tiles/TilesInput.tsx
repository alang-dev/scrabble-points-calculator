...
const TileSlot: React.FC<TileSlotProps> = ({ char, isActive, autoCapitalize = true, hasFakeCaret, ...props }) => {
  return (
    <div
      className={cn(
        ...
        render={({ slots }) => (
          <div className="flex items-center gap-3">
            {slots.map((slot, index) => (
              <TileSlot key={index} {...slot} autoCapitalize={autoCapitalize} data-testid={`tile-${index}`} />
            ))}
          </div>
        )}
