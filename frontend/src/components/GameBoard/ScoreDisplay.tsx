...
const ScoreDisplay: React.FC<ScoreDisplayProps> = ({ className }) => {
  const { isScoreComputing, scoreValue } = useGameBoard()
  return (
    <div className={`text-center ${className || ''}`}>
      <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg p-6 shadow-lg">
        <h3 className="text-lg font-semibold mb-2">Your Score</h3>
        <div
          className="text-4xl font-bold flex items-center justify-center gap-2 min-h-[3rem]"
        ...
        >
          {isScoreComputing ? (
            ...
            <span className="min-w-[4rem] text-center" data-testid="score-value">
              {scoreValue}
            </span>
          )}
        </div>
