...
public class ScoringRulesServiceImpl implements ScoringRulesService {
  ...
  private static final List<ScoringRuleDTO> SCORING_RULES = List.of(
  ...
  private static final Map<Character, Integer> LETTER_SCORES = SCORING_RULES.stream()
      .flatMap(rule -> rule.getLetters().chars()
          .mapToObj(c -> Map.entry((char) c, rule.getPoints())))
